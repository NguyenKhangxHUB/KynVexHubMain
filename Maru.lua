loadstring(game:HttpGet("https://raw.githubusercontent.com/NguyenKhangxHUB/PhKhangxHub/refs/heads/main/loadingmaru.lua"))()
task.spawn(function()
...
--[[
    Script: Auto Buy Melee Styles
    Description: Tự động mua các fighting styles dựa trên lựa chọn trong GUI.
    Yêu cầu: Script này được thiết kế để chạy cùng với UI Fluent đã có sẵn trong script "Maru.lua".
    Cách dùng: Thêm đoạn code này vào cuối script "Maru.lua" (trước dòng Window:SelectTab(1) là được).
]]

-- ==================== KHAI BÁO BIẾN TOÀN CỤC ====================
_G.AutoBuyMelee = false
_G.SelectedMelee = "Superhuman" -- Mặc định

-- ==================== DANH SÁCH MELEE VÀ NPC ====================
local MeleeData = {
    -- Tên hiển thị, Tên item trong game, Tên NPC, Vị trí NPC (CFrame)
    ["Superhuman"] = {
        ItemName = "Superhuman",
        NPCName = "Superhuman",
        NPCPosition = CFrame.new(-2451.98, 73.07, -3226.11) -- Vị trí NPC ở Sea 2 (Cần xác nhận lại)
    },
    ["Death Step"] = {
        ItemName = "Death Step",
        NPCName = "Phoeyu",
        NPCPosition = CFrame.new(6412.89, 302.92, -6887.69) -- Vị trí Phoeyu (Cần xác nhận lại)
    },
    ["Sharkman Karate"] = {
        ItemName = "Sharkman Karate",
        NPCName = "Water Fighter",
        NPCPosition = CFrame.new(-3352.90, 285.01, -10534.84) -- Vị trí Water Fighter (Cần xác nhận lại)
    },
    ["Electric Claw"] = {
        ItemName = "Electric Claw",
        NPCName = "Thunder God",
        NPCPosition = CFrame.new(-7994.98, 5761.02, -2088.64) -- Vị trí Thunder God (Cần xác nhận lại)
    },
    ["Dragon Talon"] = {
        ItemName = "Dragon Talon",
        NPCName = "Uzoth",
        NPCPosition = CFrame.new(5661.89, 1211.31, 864.83) -- Vị trí Uzoth
    },
    ["Godhuman"] = {
        ItemName = "Godhuman",
        NPCName = "Godhuman",
        NPCPosition = CFrame.new(-12543.00, 336.00, -7484.00) -- Vị trí NPC Godhuman (Cần xác nhận lại)
    },
    ["Sanguine Art"] = {
        ItemName = "Sanguine Art",
        NPCName = "Sanguine Art",
        NPCPosition = CFrame.new(-8761.31, 164.85, 6161.15) -- Vị trí NPC Sanguine Art (Cần xác nhận lại)
    }
}
...
local function TravelToSea3()
    if not IsInSea3() then
        ...
        print("Đang di chuyển đến Sea 3 (Zou)...")
        task.wait(3) -- Đợi một chút để teleport
    end
end

-- ==================== HÀM TWEEN ĐẾN VỊ TRÍ NPC ====================
...
-- ==================== HÀM MUA MELEE ====================
local function BuySelectedMelee()
    if not _G.AutoBuyMelee then return end
    if not _G.SelectedMelee then return end

    local meleeInfo = MeleeData[_G.SelectedMelee]
    if not meleeInfo then
        print("Không tìm thấy thông tin melee: " .. _G.SelectedMelee)
        return
    end
    ...
    -- Kiểm tra nếu đã có melee này rồi
    if player.Backpack:FindFirstChild(meleeInfo.ItemName) or character:FindFirstChild(meleeInfo.ItemName) then
        print("Đã sở hữu " .. meleeInfo.ItemName .. "!")
        _G.AutoBuyMelee = false -- Tắt tự động
        return
    end

    -- 1. Di chuyển đến Sea 3 nếu chưa ở đó
    TravelToSea3()

    -- 2. Tween đến vị trí NPC
    local npcPos = meleeInfo.NPCPosition
    if not npcPos then
        print("Không có vị trí NPC cho " .. _G.SelectedMelee)
        return
    end

    print("Đang di chuyển đến NPC " .. meleeInfo.NPCName .. "...")
    TweenToPosition(npcPos)

    -- 3. Tương tác với NPC để mua (Sử dụng Remote)
    -- Lưu ý: Cách mua mỗi melee có thể khác nhau, cần kiểm tra kỹ.
    -- Dưới đây là cách mua chung, có thể cần chỉnh sửa cho từng loại.
    local remote = game:GetService("ReplicatedStorage").Remotes.CommF_

    if _G.SelectedMelee == "Superhuman" then
        remote:InvokeServer("BuySuperhuman")
    elseif _G.SelectedMelee == "Death Step" then
        remote:InvokeServer("BuyDeathStep")
    elseif _G.SelectedMelee == "Sharkman Karate" then
        remote:InvokeServer("BuySharkmanKarate")
    elseif _G.SelectedMelee == "Electric Claw" then
        remote:InvokeServer("BuyElectricClaw")
    elseif _G.SelectedMelee == "Dragon Talon" then
        remote:InvokeServer("BuyDragonTalon")
    elseif _G.SelectedMelee == "Godhuman" then
        remote:InvokeServer("BuyGodhuman")
    elseif _G.SelectedMelee == "Sanguine Art" then
        remote:InvokeServer("BuySanguineArt")
    end

    print("Đã yêu cầu mua " .. _G.SelectedMelee)
end
...
task.spawn(function()
    while task.wait(1) do -- Kiểm tra mỗi giây
        if _G.AutoBuyMelee then
            pcall(BuySelectedMelee)
        end
    end
end)

-- ==================== THÊM CÁC TOGGLE VÀO UI ====================
-- Tìm tab "Fighting Style" (Melee)
local meleeTab = nil
...
if meleeTab then
    -- Thêm Dropdown để chọn Melee
    local meleeDropdown = meleeTab:AddDropdown("MeleeSelect", {
        Title = "Select Melee to Buy",
        Description = "Choose a fighting style",
        Values = {"Superhuman", "Death Step", "Sharkman Karate", "Electric Claw", "Dragon Talon", "Godhuman", "Sanguine Art"},
        Multi = false,
        Default = 1,
        Callback = function(value)
            _G.SelectedMelee = value
            print("Selected Melee: " .. value)
        end
    })

    -- Thêm Toggle để Bật/Tắt Auto Buy
    local autoBuyToggle = meleeTab:AddToggle("AutoBuyMelee", {
        Title = "Auto Buy Selected Melee",
        Description = "Automatically travel to NPC and buy the selected fighting style",
        Default = false,
        Callback = function(value)
            _G.AutoBuyMelee = value
            if value then
                print("Auto Buy Melee is ON. Target: " .. _G.SelectedMelee)
            else
                print("Auto Buy Melee is OFF.")
            end
        end
    })

    print("Đã thêm UI mua Melee vào tab 'Fighting Style'")
else
    warn("Không tìm thấy tab 'Fighting Style'. Vui lòng kiểm tra tên tab.")
end

-- ==================== KẾT THÚC ====================
print("Script Auto Buy Melee đã được tải thành công!")
