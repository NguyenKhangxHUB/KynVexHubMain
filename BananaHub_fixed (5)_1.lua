    ...
    while task.wait(0.5) do
        if _G.StartFarm and _G.MethodSelect == "Level Farm" then
            pcall(function()
                ...
                -- Tele đến submerged island nếu đủ level
                TeleportedToSub = IsInSubmergedIsland()
                if lvl >= 2600 and not TeleportedToSub then
                    repeat
                        task.wait()
                        notween(CFrame.new(SubNPC))
                ...
                -- Nếu chưa có quest → tele đến NPC rồi nhận
                if not questVisible then
                    local questPos = questData[6]  -- PosQ
                    local questName = questData[3] -- Qname
                    local questID = questData[2]   -- Qdata

                    if _G.DelayQuest then task.wait(20) end

                    if not _G.RemoteQuest then
                        -- Tele thẳng (notween) đến NPC cho chắc
                        notween(questPos)
                        task.wait(0.5)
                        -- Thử nhận quest tối đa 5 lần
                        for i = 1, 5 do
                            pcall(function()
                                replicated.Remotes.CommF_:InvokeServer("StartQuest", questName, questID)
                            end)
                            task.wait(0.3)
                            if QuestGui.Visible then break end
                        end
                    else
                        for retry = 1, 5 do
                            if QuestGui.Visible then break end
                ...
                elseif questVisible then
                    -- Kiểm tra quest title đúng không
                    ...
                    if TitleText and not string.find(TitleText.Text, questData[5]) then
                        replicated.Remotes.CommF_:InvokeServer("AbandonQuest")
                        return
                    end

                    local enemyName = questData[1] -- Mon
                    local mobPos = questData[4]    -- PosM

                    -- Tìm mob trong workspace
                    local target = nil
                    for _, v in pairs(workspace.Enemies:GetChildren()) do
                    ...
                    if target then
                        -- Farm mob cho đến khi chết hoặc quest xong
                        repeat
                            task.wait()
                            Attack.Kill(target, _G.StartFarm)
                        until not _G.StartFarm
                            or not target.Parent
                            or not Attack.Alive(target)
                            or not QuestGui.Visible
                    else
                        -- Không tìm thấy mob → tele đến vị trí mob spawn
                        notween(mobPos)
                        task.wait(0.5)
                    end
                end
